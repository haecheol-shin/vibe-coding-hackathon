...
개인 생산성 웹앱 MVP는 Python/FastAPI가 Vue.js 정적 화면과 CopilotKit SDK 엔드포인트를 함께 서빙합니다. 팀원은 프런트엔드/백엔드 역할이 아니라 기능 단위로 담당하고, 맡은 기능에 필요한 화면, API, 문서를 함께 수정합니다.
...
| 기준 | 담당 방식 | 예시 |
|------|----------|------|
| **기능** | 팀원이 기능 하나를 맡아 필요한 화면, API, 문서를 함께 구현 | 사용자 정보 입력, 감정 차트, 투두 보관함 |
| **API 계약** | 기능 담당자가 요청/응답 형식을 문서화 | `API.md` 업데이트 |
| **검증** | 기능 담당자가 로컬 실행 결과를 확인 | 화면 동작, API 응답, 오류 상태 |
| **Git** | 기능 단위로 커밋하고 푸시 | "사용자 정보 입력 기능 추가" |
